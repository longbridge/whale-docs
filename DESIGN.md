...
Six product entries per language, in **fixed order**:

**Docs → Whale Console → WhaleSDK → Broker API → Trading API → OpenAPI**
...
e.g. `whale-console/overview.mdx`, `whalesdk/overview.mdx`. A product whose docs
...
from light-only Whale surfaces such as Whale Console. The foundation treats
