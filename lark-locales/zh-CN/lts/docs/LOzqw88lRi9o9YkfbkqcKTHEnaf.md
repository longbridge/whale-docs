...
1. T0，T1 是取昨天清算文件的待交收金额，不会随著当天交易而更新。
2. 如果当天有交易的话，Total 便会更新，而 T0，T1，T2 不会更新。出现了 T0＋T1＋T2 不等于 Total 的情况，系统会显示 TN 来计算它们的差额。
3. 当天完成日终后，T0 便会消失。当天交易引致的待交收金额便会变成 T2。
...
<p>日终清算计费的时候，会解冻手续费、扣减实际手续费，有机会清算计费后的可提现金有所变更。</p>
</div>
