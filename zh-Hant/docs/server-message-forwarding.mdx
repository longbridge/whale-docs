...
| `user_info` | `object` | 透傳給 Broker App 與 WhaleSDK 的訊息資料 |
...
Broker App 從自有推送渠道取得訊息後，需要保留 `title`、`body` 和完整 `user_info`，並交給 WhaleSDK 的訊息處理入口。

```jsonc
{
    "title": "訊息標題", // WhaleSDK 需要
    "body": "訊息摘要",
    "user_info": {
        "t": "xx", // 訊息型別或埋點欄位
        "tn": "xx", // 追蹤或埋點欄位
