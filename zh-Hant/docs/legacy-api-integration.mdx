...
<Note>
Broker API 是機構級 Server-to-Server 介面，由 Broker 服務端呼叫，不應在 Broker App 中儲存 Access Key 或直接請求。
</Note>

Broker API 用於實現證券櫃檯業務，主要覆蓋使用者帳戶、資產、交易和新股等模組。完整介面參考見 [Broker API](/zh-Hant/broker-api/overview)。

<Accordion title="原始對接問題（英文）">
