...
<CardGroup cols={2}>
  ...
  <Card title="使用 Whale 使用者體系" icon="key">
    Broker 完成初始登入銜接，並將 Whale 返回的憑證交給 WhaleSDK 或 WhaleCore；後續校驗與續期由 SDK 層管理。
  </Card>
</CardGroup>
...
Broker 服務端負責完成初始使用者登入或綁定，並把 Whale 返回的初始憑證安全交給 Broker App。Broker App 隨後將 `token` 與 `refresh_token` 設定到 WhaleSDK；完全自行實現證券功能 UI 時，則設定到 WhaleCore。兩者後續都由 Whale SDK 層管理憑證。
...
<Steps>
  ...
  <Step title="設定到 Whale SDK 層">
    Broker App 在初始化時將初始憑證設定到 WhaleSDK；使用 WhaleCore 的 Broker App 則設定到 WhaleCore。Broker App 不需要自行呼叫 check token。
  </Step>
  <Step title="由 SDK 自動校驗和續期">
    WhaleSDK 或 WhaleCore 根據 token 有效期自行校驗並使用 refresh token 自動續期。正常情況下，只要續期持續成功，Broker 可以把 Customer 會話視為長期有效。
  </Step>
...
<Warning>
Broker App 不應自行實作 token 有效期判斷、check token 或 refresh token 續期，也不應與 WhaleSDK / WhaleCore 同時維護兩套刷新狀態。Broker 服務端仍必須對自己直接處理的資源執行歸屬和越權檢查。
</Warning>

下一步請參閱[使用者綁定與證券帳戶開戶](/zh-Hant/docs/account-opening)，瞭解關聯欄位、開戶狀態和介面呼叫順序。
