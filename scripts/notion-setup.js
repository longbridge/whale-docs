/*
 * 1. read notion-pages/docs.json file find first level meta slug equal zh-HK
 * 2. find the slug file in notion-pages/docs/:slug.md copy it to notion-locales/zh-HK/docs/:slug.md
 * 3. convert notion-pages/docs/:slug.md zh-CN then new file to notion-locales/zh-CN/docs/:slug.md
...
function convertHK2CN() {
  const docsPath = resolve(__dirname, "../notion-pages/docs")
  ...
  const docsMetaString = readFileSync(resolve(__dirname, "../notion-pages/docs.json"), "utf-8")
  const cnDocsMetaString = converter(docsMetaString)
  writeFileSync(resolve(__dirname, "../notion-pages/docs-zh-cn.json"), cnDocsMetaString)
  ...
  VERSIONS.forEach((version) => {
    ...
    convertedDocs.forEach(({ path, hkContent, cnContent, relativePath }) => {
      ...
      const hkFilePath = path.replace("notion-pages", `notion-locales/zh-HK/${versionPath}`)
      ...
      const cnFilePath = path.replace("notion-pages", `notion-locales/zh-CN/${versionPath}`)
...
function setupEnDocs() {
  VERSIONS.forEach((version) => {
    ...
    const versionEnMetadata = docsMeta.find((doc) => (doc.meta?.originalSlug || doc.meta?.slug) === "en")
    ...
    versionEnDocs.forEach((doc) => {
      const enFilePath = path.replace("notion-pages", `notion-locales/en/${versionPath}`)
...
function getDocMetaFromPath(docsMeta, filePath) {
  ...
  function findNode(nodes) {
    for (const node of nodes) {
      if (node.node_token === fileName || node.slug === fileName || node.originalSlug === fileName) {
        return node
      }
...
// Copy notion-pages/assets/*.png to notion-locales/assets
function setupAssets() {
  const assetsPath = resolve(__dirname, "../notion-pages/docs/assets")
  // find png or jpg or jpeg gif file
  const assets = sync(`${assetsPath}/*.{png,jpg,jpeg,gif}`)

  assets.forEach((asset) => {
    const targetAssetsPath = asset.replace("notion-pages/docs/", "notion-locales/")
...
// read notion-pages/docs.json file find slug equal guides, then copy it to notion-locales/zh-HK/docs/index.md
...
function setupIndexPage() {
  VERSIONS.forEach((version) => {
    ...
    const versionHkMetadata = versionDocs.find((doc) => (doc.meta?.originalSlug || doc.meta?.slug) === "zh-HK")
    ...
    if (guidesChild) {
      const guidesFilePath = resolve(__dirname, `../notion-pages/docs/${guidesChild.filename}`)

      if (existsSync(guidesFilePath)) {
        const guidesContent = readFileSync(guidesFilePath, "utf-8")
        const guidesTargetDir = resolve(__dirname, `../notion-locales/zh-HK/${versionPath}`)
...
function normalize_hidde_docs() {
  ...
  const all_docs = compact(
    uniq(
      sync(`${rootPath}/notion-pages/docs/*.md`).map((doc) => {
        return path.basename(doc)
      })
    )
  )

  const raw_docs_data = JSON.parse(readFileSync(resolve(__dirname, "../notion-pages/docs.json"), "utf-8"))
  ...
  const removed_docs = flattenDeep(
    all_docs
      ...
      .map((f) => {
        return [
          `${rootPath}/notion-pages/docs/${f}`,
          `${rootPath}/notion-locales/zh-CN/docs/${f}`,
          `${rootPath}/notion-locales/zh-HK/docs/${f}`,
          `${rootPath}/notion-locales/en/docs/${f}`,
          `${rootPath}/notion-locales/zh-CN/lts/docs/${f}`,
          `${rootPath}/notion-locales/zh-HK/lts/docs/${f}`,
