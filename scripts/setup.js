...
function convertHK2CN() {
  ...
  const docsMeta = readFileSync(resolve(__dirname, "../feishu-pages/docs.json"), "utf-8")
  const cnDocsMeta = converter(docsMeta)
  writeFileSync(resolve(__dirname, "../feishu-pages/docs-zh-cn.json"), cnDocsMeta)
...
function getAllNodeTokens(data, version) {
  const nodeTokens = []

  function extractNodeTokens(nodes) {
    for (const node of nodes) {
      // Skip nodes that don't match the version
      if (node.meta?.version && node.meta.version !== version) {
        continue
      }
...
function normalize_hidde_docs() {
  ...
  VERSIONS.forEach((version) => {
    ...
    const the_news_docs = compact(uniq(getAllNodeTokens(raw_docs_data, version)))
      ...
      all_docs
        ...
        .map((f) => {
          return [
            ...
            `${rootPath}/locales/zh-CN/${versionPath}docs/${f}`,
            `${rootPath}/locales/zh-HK/${versionPath}docs/${f}`,
            `${rootPath}/locales/en/${versionPath}docs/${f}`,
          ]
        })
    )
    ...
    removed_docs.forEach((doc) => {
      if (fs.existsSync(doc)) {
        fs.unlinkSync(doc)
        console.log(`--> removed file (${version}): ${doc}`)
      } else {
        console.log(`--> not found file (${version}): ${doc}`)
      }
    })
  })
}
