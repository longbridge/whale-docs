...
- **多條件查詢**：支持按記錄編號、客戶編號、創建時間、狀態、等條件篩選查詢，快速定位目標記錄。
- **詳情跟蹤**：點擊單條記錄查看詳情，包含地址核對、交易哈希對應的區塊確認進度等信息，實時跟蹤入幣全流程。
...
- **操作前提**：僅針對狀態為「錢包地址認證中」的記錄（即區塊鏈網絡未開始確認、資產未轉出的入幣請求），其他狀態記錄不可撤銷。
- **操作步驟**：
...
- **操作前提**：僅針對狀態為「已到賬」的記錄（即資產已成功入賬至平台，用戶申請退幣或系統需強制退幣場景），需核實用戶身份及退幣合理性。
- **操作步驟**：
