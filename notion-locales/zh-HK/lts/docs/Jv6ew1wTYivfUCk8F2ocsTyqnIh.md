...
| 融資總金額        | 是        | 用於限制所有客戶可使用融資的最大總金額 例如設置融資總金額設定為 500,000 HKD，融資比例為 90%。 客戶 A 使用了 200,000 HKD 融資 客戶 B 使用了 210,000 HKD 融資 那麼此時融資池只剩下 90,000 HKD 額度，客戶使用 90%的融資進行申購，最多只能申購 90,000 /90% =100,000 HKD 的金額，申購完畢後，融資池已用額度 500,000 HKD，其他客戶將不可用再使用該融資申購，除非券商將融資額度調高。 |
| 融資比例         | 是        | 代表提供給客戶的融資比例 例如融資比例為 90%，那麼客戶 IPO 認購金額為 20,000 HKD 時，會用到 18,000 HKD 的 IPO 融資。 支持融資比例選擇 可自定義融資比例，設置好系統自動提醒槓桿倍數                                                                                                                                |
| 認購時間         | 是        | 代表此認購方式的可認購時間 券商可以靈活使用這個時間，做好 IPO 運營管理，例如可以配置為： 1 月 3 日支持 20 倍打新（2024-01-03 00:00:00  ～  2024-01-04 00:00:00） 1 月 4 日支持 10 倍打新（2024-01-04 00:00:00  ～  2024-01-05 00:00:00）                                                                  |
...
| 渠道融資比例       | 否        | 僅作為記錄使用，代表上手或銀行提供了多少融資比例。 例如上手只給了 80%的融資比例，但券商想給客戶提供 90%融資，那麼可設置為： 融資比例 = 90% 渠道融資比例 = 80% 系統在統計時，能更方便的計算出券商自己墊付了 10%的融資                                                                                                                     |
