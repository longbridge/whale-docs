...
| 訂單狀態   | 待提交：非交易時段預埋單、港股價格保護的預埋單或未觸發的條件單，可操作改單、撤單 待委託：訂單提交給交易所後，部分上手返回的狀態，可操作改單、撤單 已委托：訂單提交給交易所後，訂單被接收掛單中，可操作改單、撤單 已提待报：訂單提交給交易所後，未收到交易所回覆，不可操作改單、撤單 已成交：發給交易所後，全部成交 部分成交：訂單部分成交部分掛單中，可操作改單、撤單 已拒绝：訂單被交易所或被交易 RMS 風控拒絕 已撤单：用戶手動撤單完成 已过期：訂單已過期 |
...
![到價買入](/assets/f48615c99182da6563447a596cdf81c8.png)
