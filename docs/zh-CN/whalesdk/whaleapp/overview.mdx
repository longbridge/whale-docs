...
WhaleAppSDK for iOS 和 WhaleAppSDK for Android 向 Broker App 提供完整的证券业务图形界面。Broker App 不需要重新实现行情、资产和交易页面，主要负责集成 SDK、传入 Customer 会话、承接系统能力及管理 App 生命周期。
...
## 可选：行为埋点转发 {/* optional-analytics-forwarding */}

部分项目可以申请将 WhaleAppSDK 内部埋点事件回调给 Broker App。该能力不是默认公共接口，需要 Whale 项目团队确认 SDK 版本和授权范围。
...
- WhaleAppSDK 自动续期无需 Broker App 介入；不可恢复的 token 失效、异地登录和 Customer 切换均有明确处理。
