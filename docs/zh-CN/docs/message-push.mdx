...
Whale 的订单、账户及其他业务消息可以通过两种方式送达客户：由 Whale 管理云推送渠道并直接投递到设备，或先通过 Server-to-Server 接口转发给 Broker，再由 Broker 自行决定是否通知及如何分发。

## 完整接入文档 {/* detailed-guides */}
...
### Android 渠道信息 {/* android-channel-information */}
...
### iOS APNs 信息 {/* ios-apns-information */}
...
sequenceDiagram
    ...
    Center->>Channel: 投递到目标客户的设备
    Channel-->>App: title、body、user_info
    App->>App: 将标准消息体交给 WhaleAppSDK
```

### Broker 提供的信息 {/* information-from-broker */}
...
| 字段 | 说明 | Broker 处理建议 |
| --- | --- | --- |
| `member_ids` | Whale 客户标识列表 | 映射到 Broker 用户和设备，不要直接作为展示内容 |
...
### 服务端实现要求 {/* broker-server-recommendations */}
...
## 客户端实现 {/* broker-app-implementation */}
...
## 联调与验收 {/* integration-and-acceptance */}

至少覆盖以下场景：

- UAT 与生产 App 标识、凭证和消息数据不会串用。
- 单个与多个 `member_ids` 均能正确映射目标客户。
- 同一 `post_id` 重复投递只产生一次用户可见通知。
- App 位于前台、后台、被系统终止时均有明确行为。
- 客户登出、切换账户或关闭通知权限后不会错误投递。
