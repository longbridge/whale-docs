...
| [WhaleCore + TradingAPI](/zh-cn/trading-api/overview) | Broker App | 客户 | iOS / Android / Web 数据 SDK + HTTP API | Broker App 完全自行实现证券功能 UI | iOS / Android / Web（WebAssembly）可用 |
...
- **完全自行实现证券功能 UI**：使用 **WhaleCore + TradingAPI**。WhaleCore 提供行情订阅、WebSocket 连接、认证签名和 token 续期等客户端基础机制；TradingAPI 通过 HTTP 提供账户、资产和交易等业务能力。
...
flowchart TD
    ...
    C -- Web --> F[WebTrade]
    B -- 否，完全自行实现 UI --> G[WhaleCore + TradingAPI]
...
- **客户授权**：WhaleAppSDK（包括 WebTrade）、WhaleCore、TradingAPI 和 OpenAPI 均以单个客户的权限访问私有数据。
- **Broker Developer 与开发者**：前者代表 Broker 建设产品；后者是 Broker 的客户，使用 OpenAPI 开发策略交易、量化分析或 AI 应用。两者不共享凭证和授权范围。

<Note>
WhaleCore 不提供任何图形界面，也不是 TradingAPI 的替代品。完全自行实现证券功能 UI 的 Broker App，通常同时使用 WhaleCore 的长连接与基础机制，以及 TradingAPI 的 HTTP 业务接口。
</Note>
