...
<Steps>
  ...
  <Step title="双方完成消息联调">
    建议验证正常消息、重复 `post_id`、鉴权失败、接口超时、返回失败和多客户消息；实际范围由双方确认。
  </Step>
  <Step title="Broker App 验收">
    验证前台、后台、冷启动、客户登出、通知权限关闭和安全跳转。
  </Step>
</Steps>

## HTTP 接口约定 {/* http-endpoint-convention */}
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `member_ids` | `string[]` | Whale 客户标识列表，Broker 需映射到自己的用户与设备 |
...
| `pre_params` | `object` | Whale 内置预设变量，可能包含客户资料 |
| `user_info` | `object` | 透传给 Broker App 与 WhaleAppSDK 的消息数据 |

### 通用请求示例 {/* generic-request */}
...
## 完整订单消息示例 {/* complete-order-message-example */}
...
## SDK 消息对接格式 {/* whaleappsdk-message-format */}

Broker App 从自有推送渠道取得消息后，需要保留 `title`、`body` 和完整 `user_info`，并交给 WhaleAppSDK 的消息处理入口。

```jsonc
{
    "title": "消息标题", // WhaleAppSDK 需要
    "body": "消息摘要",
    "user_info": {
        "t": "xx", // 消息类型或埋点字段
        "tn": "xx", // 追踪或埋点字段
...
## 已有模板文件 {/* existing-template-files */}
