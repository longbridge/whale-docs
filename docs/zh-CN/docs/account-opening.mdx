...
<CardGroup cols={3}>
  <Card title="登录与绑定" icon="link" />
  <Card title="提交开户申请" icon="file-pen" />
  <Card title="查询申请状态" icon="rotate" />
</CardGroup>
...
## 绑定关系与状态模型 {/* mapping-and-status-model */}

| 标识 | 所属系统 | 用途 | 建议保存位置 |
| --- | --- | --- | --- |
| `open_id` | Broker | 客户在 Broker 用户体系中的稳定唯一标识 | Broker 服务端 |
...
<Warning>
`open_id` 必须稳定且不可被其他客户复用。Broker 应在服务端校验 `member_id`、`application_id` 和 `account_no` 的归属关系，不能信任 Broker App 直接传入的关联标识。
</Warning>

## 详细设计 {/* detailed-design */}

### 总体流程 {/* end-to-end-flow */}
...
### 用户注册登录 {/* register-or-sign-in-a-user */}

`POST /v1/whaleapi/auth/open_id/login`

##### 请求字段说明 {/* request-fields */}
...
### 提交开户申请 {/* submit-an-account-opening-application */}

`POST /v2/whaleapi/account/submit_application`

##### **请求字段说明** {/* request-fields-1 */}

| 字段名 | 类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| `member_id` | integer | Y | 用户 ID |
| `client_type` | integer | Y | 客户类型：`1` 表示个人户，`2` 表示企业户 |
| `open_info` | object | Y | 开户表单信息 |
| `+ account_no` | string | N | 证券账号 |
| `+ mark` | string | N | 账户标记 |
| `+ upload` | object | Y | 证件信息 |
| `++ card_country` | string | Y | 证件签发国家或地区；参阅 [ISO 3166-1 alpha-2](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2#Officially_assigned_code_elements) |
| `++ card_type` | integer | Y | 证件类型：`0` 表示身份证，`1` 表示护照 |
| `++ card_id` | string | Y | 证件号码 |
| `++ first_name` | string | N | 英文名或名字拼音 |
| `++ last_name` | string | Y | 英文姓或姓氏拼音 |
| `+ base` | object | Y | 个人基本信息 |
| `++ residence_country` | string | Y | 居住国家或地区；参阅 [ISO 3166-1 alpha-2](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2#Officially_assigned_code_elements) |
| `++ residence_part` | string | N | 居住区域 |
| `++ residence_desc` | string | Y | 详细居住地址 |
| `+ employ` | object | N | 职业信息 |
| `++ contact` | object | Y | 联系方式 |
| `+++ emails` | array \[Email\] | Y | 邮箱地址，包括结单接收邮箱 |
| `++ taxes` | array \[Tax\] | Y | 各国家或地区的税务信息 |
| `+ finance` | object | Y | 财务信息 |
| `++ account_type` | string | Y | 账户类型：`C` 表示现金账户，`M` 表示融资账户 |
| `+ risk` | object | N | 风险信息 |
| `++ risk_tolerance` | object | N | 风险测评信息 |
| `+++ audit` | bool | Y | 风险测评是否通过 |
| `+++ end_at` | integer | Y | 风险测评过期时间 |

#### 请求与响应示例 {/* request-and-response-examples-1 */}
...
### 获取开户申请信息 {/* get-account-opening-application-information */}

`POST /v1/whaleapi/account/get_application`

##### **请求字段说明** {/* request-fields-2 */}

| 字段名 | 类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| `application_id` | string | Y | 开户申请 ID |
| `member_id` | string | 待确认 | 原始请求示例包含该字段，但原始字段表未声明；请以当前 BrokerAPI 契约为准 |

##### 响应字段 {/* response-fields */}

| 字段名 | 类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| `code` | integer | Y | 业务状态码 |
...
| `+ base` | object | — | 基本信息 |
| `++ country_code` | string | Y | 出生国家或地区 |
| `++ birth_date` | string | Y | 出生日期 |
| `++ gender` | string | N | 性别：<br />- `male`：男性<br />- `female`：女性 |
| `++ marital_status` | string | N | 历史字段表记录的婚姻状态值：`single`、`married`、`other`、`divorced` 或 `widowed`。实施前请根据当前 BrokerAPI 契约确认具体含义。 |
| `++ residence_country` | string | Y | 居住国家或地区 |
| `++ residence_part` | string | Y | 居住区域 |
| `++ residence_desc` | string | Y | 详细居住地址 |
| `+ employ` | object | — | 职业信息 |
| `++ type` | string | N | 职业类型 |
...
#### 请求与响应示例 {/* request-and-response-examples-2 */}
...
| `401033` | 用户冻结 | 阻止继续操作并向客户展示账户状态 |
| `401040` | 用户已注销 | 清理本地会话和不可用账户状态 |
| `401047` | 系统基础配置缺失 | 联系 Whale 项目团队补充配置 |
| `404002` | 身份信息已存在，请使用原账号进行证券交易 | 引导客户使用原账户 |
...
## 常见问题与待确认项 {/* faqs-and-items-to-confirm */}

<AccordionGroup>
  <Accordion title="如何打通两边的用户？token 和 refresh_token 如何获得？">
    使用稳定的 `open_id` 调用 [用户注册登录](#register-or-sign-in-a-user) 接口。接口返回 `member_id`、`token` 和 `refresh_token`；Broker 保存用户映射，并将初始凭证交给 Broker App 设置到 WhaleAppSDK 或 WhaleCoreSDK。
  </Accordion>
  <Accordion title="开户资料字段如何填写？">
    请参阅 [BrokerAPI 开户字段文档](https://apidocs.longbridgewhale.com/whaleapi/submit-application-v2%E6%8F%90%E4%BA%A4%E5%BC%80%E6%88%B7%E7%94%B3%E8%AF%B7%E8%B5%84%E6%96%99-3469908e0)。
  </Accordion>
  <Accordion title="Token 是否区分客户端？">
    不区分。取得初始 token 后应提供给 Broker App，并设置到 WhaleAppSDK 或 WhaleCoreSDK。后续有效性检查和自动续期由 SDK 层管理，Broker App 不自行调用 check token 或实现刷新。任何一方都不能在日志中记录完整 token。
  </Accordion>
</AccordionGroup>
