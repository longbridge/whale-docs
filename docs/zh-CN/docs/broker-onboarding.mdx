...
flowchart TD
    ...
    D -->|自行实现证券功能 UI| F[WhaleCore + TradingAPI]
...
| 阶段 | 指南 |
| --- | --- |
| 理解客户、用户、开户申请、账户和消息的关系 | [核心对象与业务流程](/zh-cn/docs/concepts) |
| 确定 UI、客户端和服务端方案 | [接入方式选择](/zh-cn/docs/integration-options) |
| 确定客户注册、登录和会话归属 | [用户系统接入](/zh-cn/docs/user-system-integration) |
| 集成完整 UI | [WhaleAppSDK（iOS、Android、WebTrade）](/zh-cn/whalesdk/whaleapp/overview) |
| Broker App 自行实现证券功能 UI | [WhaleCore 与 TradingAPI](/zh-cn/trading-api/overview) |
...
| Broker App Developer | WhaleAppSDK（iOS、Android、WebTrade）或 WhaleCore 集成 |
...
| 工作 | Broker | Whale |
| --- | --- | --- |
| Broker 产品范围和客户体验 | 决策并验收 | 提供能力边界与实施建议 |
...
- **接入边界**：采用 WhaleAppSDK、WhaleCore + TradingAPI 或 BrokerAPI；由哪一方维护客户用户体系。
...
- Broker App 使用 WhaleAppSDK（iOS、Android、WebTrade），还是使用 WhaleCore + TradingAPI 自行实现证券功能 UI。
- Broker Server 需要使用哪些 BrokerAPI 能力。
- 客户用户由谁管理，`open_id` 如何生成和保持稳定。
- 开户申请如何提交、查询和展示失败结果。
