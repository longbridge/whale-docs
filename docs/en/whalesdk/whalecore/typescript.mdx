...
Signing credentials are not configuration fields; the key pair is compiled into the binary. The engine ignores unrecognized keys, so a misspelled field never fails; it takes no effect. Let the `EngineConfig` type check the object.
