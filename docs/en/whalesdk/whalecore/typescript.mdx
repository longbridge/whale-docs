...
One runtime dependency follows: `@bufbuild/protobuf`, because quote payloads are Protobuf. It is pure ESM and pulls in nothing transitively. Builds that only use `initialize` and `request` drop the decoder.
...
The package declares `sideEffects: false`, so a bundler may drop the engine-bound exports an application never calls.

## Host the WebAssembly binary {/* host-the-wasm-binary */}

`engine_bg.wasm` is fetched at runtime by URL, and no bundler inlines it. The binary sits in the package's `exports` map, so the bundler can resolve and fingerprint it — Vite with `?url`, webpack 5 with `asset/resource`, esbuild with the `file` loader — and `load` takes the result:
...
Copying the file into the served assets by hand works too, and so does any CDN URL. Serve it with `Content-Type: application/wasm`. Without an explicit `load`, the engine looks for the file next to its own JavaScript module, which breaks once the bundler moves output. `load` is idempotent and safe to retry, and `initialize` calls it when the host does not.

### Compress the binary {/* compress-the-binary */}

The published binary is already optimized for size, so transport is the remaining cost: Brotli quality 11 removes roughly 69% of it, where `gzip -9` stops near 58%. Compress during the build rather than per request, with `vite-plugin-compression2`, `compression-webpack-plugin`, or whatever the bundler provides.
...
## Share one engine across tabs {/* share-across-tabs */}

One SharedWorker can own the engine for every tab of the application, while the interfaces themselves load no WebAssembly at all — one connection and one quote store instead of a copy per tab.

What makes that practical is that the values a screen needs in order to describe what it wants, and to read what it is told, no longer come with the engine attached. `whalecore-web/constants` makes one promise: the engine is not in its runtime graph. It carries `ReadyState`, `SubType`, `Change`, `WatchlistChange`, the watchlist group identifiers, `TradeStatus`, `StockTemplate`, `Http`, and a `*Name()` helper for each code table. A badge that renders a connection state costs a few integers instead of the whole binary. The same values are still exported from their own modules, so existing imports keep working.

Reverse lookup changed along with it. `TradeStatus`, `StockTemplate`, and `ReadyState` are declared in TypeScript now, typed `number` because the value comes from the server. Literal indexing such as `TradeStatus[105]` still works, but a variable needs `tradeStatusName(code)`, `stockTemplateName(code)`, or `readyStateName(state)`.
...
| Switch the current account | `portfolio.setAccount(accountChannel, aaid)`. Later requests are scoped to that channel |
| Switch Client, same tenant | `portfolio.logout()`, write the new `token` and `refresh_token`, `portfolio.login()`, discover accounts again, resubscribe |
...
| `whalecore-web/constants` | Bit flags and code tables with no engine behind them, see [Share one engine across tabs](/en/whalesdk/whalecore/typescript#share-across-tabs) |
| `whalecore-web/quotes` | Quote store with subscriptions, snapshots, and push merging; one-shot queries (`quoteDetail`, `quoteKline`, minutes, trades, order book, adjust factors, interest rate); `Counter`, `Decimal` helpers, derived metrics, and `greeks` |
...
- The subpath also exports one-shot fetches — `quoteDetail`, `quoteList`, `quoteDepth`, `quoteKline`, `quoteTrade`, `quoteMinute`, `queryAdjustFactor`, `queryInterestRate` — which return a payload once and start no subscription. Live data still comes from `mount` and `watch`.
...
| `setAccount(accountChannel, aaid)` | Switch the session's current account |
