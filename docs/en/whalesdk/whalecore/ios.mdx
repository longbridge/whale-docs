---
title: iOS
description: WhaleCore iOS system requirements, initialization, and the quotes, order, portfolio, watchlist, and pass-through request services
---
...
**Trade authentication is fully automatic by default** and invisible to the host: for trade-scoped calls (submitting orders, replacing or canceling orders, trade pass-through requests, and so on), the SDK obtains and recovers trade tokens internally without any configuration.

When the integration requires the host to verify the trading password (pass `tradePasswordEnabled: true` at initialization; the Swift parameter can be omitted and defaults to `false`), the SDK no longer obtains trade tokens itself: a trade-scoped call without a valid token throws `tradeAuthFailed` (1006). The host then prompts the Client for the trading password, exchanges it for tokens through its own API, hands them to the SDK, and reissues the original operation itself — the SDK does not replay it.

```swift
// Host presents the trading-password UI, calls its own trade-auth API, and receives both token values
try WhaleCoreService.updateTradeToken(
    tradeToken,
    tradeRefreshToken: tradeRefreshToken,
    accountChannel: accountChannel   // omit to use the defaultAccountChannel from initialization
)
```
...
[WhaleCoreService updateTradeToken:tradeToken
                tradeRefreshToken:tradeRefreshToken
                   accountChannel:accountChannel   // pass nil for the default account
                            error:&error];
```

When catching 1006, read the affected account from the error so the tokens go to the right account. Before opening a trading screen you can also check whether a token is already held locally, so the Client is not interrupted after filling in an order:

```swift
if error.whaleCoreErrorCode == .tradeAuthFailed {
    let channel = error.whaleCoreAccountChannel   // affected account; pass it to updateTradeToken
}

if !WhaleCoreService.hasTradeToken() {           // omit the parameter to check the default account
    presentTradePasswordFlow()                   // verify, then updateTradeToken
}
```

```objc
if (![WhaleCoreService hasTradeTokenWithAccountChannel:nil]) { /* prompt for verification */ }
```

<Note>
  `hasTradeToken` returning `true` only means a token is held locally, not that the server still accepts it: the token may already have expired server-side and the trading API still throws `tradeAuthFailed`. It saves one failed round trip and does not replace the 1006 error branch; it returns `false` whenever the SDK is not initialized or the account channel cannot be resolved. Under the default integration (`tradePasswordEnabled` is `false`), do not use it to decide whether verification is needed: the SDK does not pre-warm trade tokens, so none exists when the screen opens and one is obtained automatically on the first trade request.

  `updateTradeToken` overwrites the account's existing tokens without comparing them, and data already on screen is not refetched — the host refreshes the affected business itself (for example, calling `refresh()` for portfolio data). It throws `invalidParameter` (3007) when either token is an empty string, or when no account channel is given and the default account channel is also an empty string. Tokens live in memory only and must be supplied again after every SDK initialization. While the SDK renews them with `tradeRefreshToken`, the host notices nothing and does not verify the password again; renewed tokens are not handed back, so the host never needs to hold or persist them — check with `hasTradeToken` and re-verify to overwrite when one is missing.
</Note>
...
class PortfolioHandler: NSObject, WhaleCorePortfolioServiceDelegate {
    ...
    func onError(error: Error) {
        // Resolve the error code in one step; typed accessors locate the channel and aaid
        if error.whaleCoreErrorCode == .tradeAuthFailed {
            print("Trade token invalid: \(error.whaleCoreAccountChannel ?? "")/\(error.whaleCoreAccountAAID ?? "")")
            return
        }
...
| `setSubscribedAccounts(accountChannels:`<br />`currency:completion:)` | Objective-C variant of the same name that reports failure: the result arrives on the main thread through completion, and failures are not dispatched again through onError |
...
| `refresh(completion:)` | Objective-C variant of the same name that reports failure: the submission result arrives on the main thread through completion |
...
Every error is thrown as an `NSError` with the domain `WhaleCoreErrorDomain`; error codes are defined in `WhaleCoreErrorCode`. Any `Error` resolves directly: `error.whaleCoreErrorCode` returns the matching code and `nil` for errors that do not come from WhaleCore.
...
do {
    ...
    case .tradeAuthFailed:      // 1006
        promptReauth()
    default:                    // nil means the error is not from WhaleCore
        log(error)
    }
}
```

Objective-C uses the `NSError` extensions instead of a hand-written domain literal:

```objc
if ([error isWhaleCoreError:WhaleCoreErrorCodeTradeAuthFailed]) {
    [self promptReauth];
} else if (error.isWhaleCoreError) {      // domain only, then branch on code
    switch (error.code) { /* ... */ }
}
```

Errors that carry a server response include the HTTP status code, business code, response body, and trace id, and account-scoped push errors carry the channel and aaid. Both groups expose typed properties of the same names (in Swift, `error.whaleCoreHTTPStatusCode` (`Int?`), `error.whaleCoreAccountChannel`, and so on; in Objective-C the numeric keys are nullable `NSNumber`), so no string keys are needed. `whaleCoreBusinessCode` is the server-side business number and is a different field from `NSError.code`, the WhaleCore error code.

Task cancellation (`CancellationError`) propagates unchanged and stays outside the unified domain: cancellation is control flow the caller started, not a business error.

| Segment | Range | Module |
| --- | --- | --- |
| General / portfolio | 1001–1009 | SDK initialization, member settings, trade authentication, the quotes service, internal exchange-rate conversion (1008 is an unavailable rate, which hosts rarely receive directly), and the watchlist service not being initialized (1009) |
...
- Switch the request language at runtime: `WhaleCoreService.setLanguage(_:)` (callable from both Swift and Objective-C, without reinitializing). This is the only runtime entry point for changing the language: requests issued after the call use the new language, in-flight requests are unaffected, and switching to the current language does nothing. The startup language comes from the `language:` parameter of the `WhaleCoreConfig` initializer; `WhaleCoreConfig.language` is read-only and always reports the language in effect. The call has no effect before the SDK is initialized
