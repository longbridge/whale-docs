---
title: Overview
description: Shared integration flow for WhaleAppSDK on iOS and Android
---

WhaleAppSDK for iOS and Android provides a complete graphical securities experience. Broker App integrates the SDK, supplies the Client session, connects operating-system capabilities, and manages the App and SDK lifecycle.

```mermaid
flowchart LR
    A[Client signs in to Broker App] --> B[Broker Server obtains Whale credentials]
    B --> C[Broker App initializes WhaleAppSDK]
    C --> D[WhaleAppSDK validates credentials and loads the account]
    D --> E[Broker App opens a WhaleAppSDK page]
    E --> F[Client uses market-data and trading UI]
    F --> G{Logout or unrecoverable session failure?}
    G -- No --> F
    G -- Yes --> H[Destroy WhaleAppSDK and clear the session]
```
...
- The Server-to-Server flow used to obtain the Client's initial `token` and `refreshToken`.
...
3. **Sign in the Client:** after Broker sign-in, obtain the Whale session from Broker Server.
...
| --- | --- |
| Multilingual App name | Name displayed inside WhaleAppSDK |
| `appKey` / `appSecret` / `appId` | Identify the project and environment |
| `token` / `refreshToken` | Current Client's Whale session |
...
| Event | Handling |
| --- | --- |
| Start succeeds | End the loading state and enable WhaleAppSDK entry points |
| Start fails | Log a sanitized error; fix configuration errors or require a new sign-in for authentication errors |
| Runtime error | Destroy the SDK and reestablish the Client session after unrecoverable authentication failure |
| Token renews | WhaleAppSDK manages validity and subsequent renewal; Broker App does not call check-token or refresh-token APIs |
...
<Warning>
Never log `appSecret`, complete tokens, refresh tokens, or Client-sensitive data. Never execute an external URL without validating it.
</Warning>

## Message push {/* message-push */}

Both platforms support Whale-managed and Broker-managed push. See [Message-push integration](/en/docs/message-push) for server-channel selection, cloud-service setup, and the standard message shape. Platform methods are documented in [iOS integration](/en/whalesdk/whaleapp/ios) and [Android integration](/en/whalesdk/whaleapp/android).

## Optional analytics forwarding {/* optional-analytics-forwarding */}

Current SDK versions expose analytics-event callbacks on both platforms. Consume only events and properties agreed for the project, and do not depend on undocumented internal business events. See [Optional APIs](/en/whalesdk/whaleapp/optional-apis).
...
- Automatic token renewal requires no Broker App intervention; unrecoverable failure, remote login, and Client switching have explicit handling.
