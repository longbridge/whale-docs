...
4. Add the following system frameworks to **Build Phases → Link Binary With Libraries**:
...
| `401` | Unrecoverable authentication failure | Destroy the SDK when needed and require Client sign-in |
...
The SDK has no dedicated method for this. Set the `theme` property on `LBWhaleApp.config` directly; the change takes effect immediately:
...
Whale delivers messages to Broker App through the SDK's built-in Aliyun + APNs channel. Broker App only starts the push service and reports the device token; the SDK owns notification display and click routing after startup (`NotificationErrorNotStarted` means the SDK must be started before routing).
...
Common causes of `NotificationErrorPushStartFailed`: an incorrect push key or secret, or a bundle ID that does not match the push console configuration.
...
These dependency-conflict fixes apply only to [CocoaPods integration](#cocoapods-integration). If the project uses manual integration instead, contact the Whale project team for the equivalent fix.
