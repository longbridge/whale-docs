...
/// 9. Respond to an H5 call into Broker App native capabilities
///
/// When it fires: an H5 page inside the SDK calls a Broker App native method (for example, generating a share short link)
///                through the `lbCallSourceApp` JSBridge. The SDK dispatches on the main thread and only forwards the call:
///                it does not check methodName against an allowlist, parse the returned business code, or apply a timeout.
/// Parameters:
///   - request.methodName : the native method name requested by H5; Broker App decides whether it is supported
///   - request.params     : the business parameters from H5, forwarded unchanged
///   - request.sourceURL  : the URL of the calling H5 page, filled in by the SDK, usable for validation and analytics
///   - completion         : Broker App must call it exactly once, from any thread; the SDK switches to the main thread
///                          before forwarding to H5. Broker App and H5 agree on the meaning of result.code / message / data,
///                          and result.data must be a JSON-serializable NSArray or NSDictionary
/// Suggested handling:
///   - dispatch by methodName to the JSBridge capabilities Broker App already has; report an unsupported methodName
///     to H5 through result.code (for example, code=1 for an invalid API)
///   - when this method is not implemented, the SDK replies to H5 directly with status=NO, code=-1, error="客户端未实现"
...
Validate the scheme, domain, and route allowlist for any URL forwarded through `lbWhaleAppSDKDidRequestOpenURL:` before opening it. Calls forwarded through `lbWhaleAppRequestSourceAppAPI:` are equally unchecked by the SDK: validate `request.sourceURL` and keep `request.methodName` on an allowlist before dispatching.
...
/// SDK analytics event callback; Broker App can feed it into its own analytics system
- (void)lbWhaleAppAnalyticsEventWithName:(NSString *)name properties:(NSDictionary *)properties;

/// H5 requests a Broker App native capability through the SDK (H5-side JSBridge: lbCallSourceApp)
/// The SDK dispatches on the main thread; completion may be called from any thread but exactly once,
/// and the SDK switches to the main thread before forwarding the result to H5
/// When this method is not implemented, the SDK replies to H5 directly with status=NO, code=-1, error="客户端未实现"
- (void)lbWhaleAppRequestSourceAppAPI:(LBWhaleAppSourceAppRequest *)request
                           completion:(void (^)(LBWhaleAppSourceAppResult *result))completion;
...
- Cold start, warm start, and repeatedly opening WhaleAppSDK pages all work correctly.
- No call to `pushURL:`/`presentURL:` or other state-dependent methods happens before `lbWhaleAppDidFinishLaunching` fires.
- On a 401 authentication failure (remote login, refresh-token expiry), `logoutAndDestroy` has been called and the Client is redirected to sign in again; WhaleAppSDK's automatic renewal requires no Broker App intervention.
- Theme, language, and price-color switching (via the corresponding `LBWhaleApp.config` properties) meet product requirements.
- Notification-tap navigation has been verified across cold start, foreground, and background, for both the SDK's built-in push and Broker's own push.
...
- Pages that require landscape orientation correctly forward it via `requiredAppInterfaceOrientationMask`.
- Logs never contain `appSecret`, a complete token, or refresh token; URLs received via `lbWhaleAppSDKDidRequestOpenURL:` are validated against a scheme/domain/route allowlist before use.
