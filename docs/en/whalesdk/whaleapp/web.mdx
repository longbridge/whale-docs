...
- WebTrade reads those cookies and signs the Client in automatically.
...
1. Client signs in to Broker website; Broker Server returns `token`, `refresh_token`, and `app_id`.
2. Broker frontend writes them to the agreed root-domain cookies.
3. Client opens WebTrade in a new tab.
...
1. Client signs in and Broker writes the session cookies.
2. Broker creates the iframe and passes `tenant_origin` in its URL.
3. WebTrade reads the cookies and signs the Client in.
...
1. Client starts logout in WebTrade.
...
1. Client starts logout when the control is not hidden.
2. WebTrade clears its login cookies and local storage.
...
| --- | --- | --- |
| `{cookie_token_key}` | Yes | Client token |
| `{cookie_refresh_token_key}` | Yes | Client refresh token |
...
2. Clear every WebTrade login cookie when the Client signs out.
3. Call the Whale logout API so the server-side Client token becomes invalid.
