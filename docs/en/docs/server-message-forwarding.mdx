...
description: Server-to-Server forwarding, complete request example, and WhaleAppSDK message format
---
...
<Steps>
  ...
  <Step title="Both parties test messages">
    Recommended cases include normal messages, repeated `post_id`, authentication failure, endpoint timeout, failure response, and multiple Clients. Both parties confirm the actual scope.
  </Step>
  <Step title="Broker App acceptance">
    Test foreground, background, cold start, Client logout, disabled permission, and safe navigation.
  </Step>
</Steps>

## HTTP endpoint convention {/* http-endpoint-convention */}
...
| --- | --- | --- |
| `member_ids` | `string[]` | Whale Client identifiers that Broker maps to users and devices |
...
| `pre_params` | `object` | Whale preset variables that may contain Client data |
| `user_info` | `object` | Pass-through data for Broker App and WhaleAppSDK |
...
```json
{
  ...
  "pre_params": {
    "key": "value",
    "member_real_name": "Example Client"
  },
  "user_info": {
    "t": "<message-type>",
    "tn": "<trace-id>",
...
## Complete order-message example {/* complete-order-message-example */}
...
```json
{
  ...
  "pre_params": {
    ...
    "account_real_name": "Example Client",
...
## WhaleAppSDK message format {/* whaleappsdk-message-format */}

Broker App retains `title`, `body`, and complete `user_info` from its own push channel and passes them to WhaleAppSDK.

```jsonc
{
  "title": "Message title", // Required by WhaleAppSDK
  "body": "Message summary",
  "user_info": {
...
## Existing template files {/* existing-template-files */}
