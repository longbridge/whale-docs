...
| **WhaleSDK** | The umbrella name for Whale client SDK products, containing the graphical WhaleAppSDK and the UI-free WhaleCore. |
| **WhaleAppSDK** | The graphical SDK in WhaleSDK, delivered as iOS, Android, and WebTrade form factors. |
| **WhaleCore** | The UI-free data SDK in WhaleSDK, targeting iOS, Android, and Web, all available; the Web form is WebAssembly in the browser. It provides market-data subscriptions, WebSocket connectivity, request signing, and token renewal. |
...
| **TradingAPI** | The client-scoped HTTP API used by a Broker App with WhaleCore when the Broker builds the securities UI from scratch. |
...
| **Broker App Developer** | A developer integrating WhaleAppSDK (iOS, Android, or WebTrade) or WhaleCore into Broker App. |
