...
| --- | --- | --- | --- |
| `open_id` | Broker | Stable unique identifier for a Client in Broker's user system | Broker Server |
...
<Warning>
`open_id` must be stable and must never be reused by another Client. Broker Server must verify the ownership relationships among `member_id`, `application_id`, and `account_no`; it must not trust related identifiers supplied directly by Broker App.
</Warning>
...
sequenceDiagram
    ...
    Client->>Broker: Register, sign in, or bind account
    ...
    Broker-->>Client: Display account-opening result
```
...
| `client_type` | integer | Y | Client type: `1` individual, `2` corporate |
...
| `++ marital_status` | string | N | Marital-status value recorded in the historical field table: `single`, `married`, `other`, `divorced`, or `widowed`. Confirm the current meanings with the BrokerAPI contract before implementation. |
...
| `401033` | User frozen | Block further operations and show the account status to the Client |
...
| `404002` | Identity information already exists; use the original account for securities trading | Guide the Client to the original account |
...
<AccordionGroup>
  <Accordion title="How are the two user systems connected, and how are token and refresh_token obtained?">
    Call the [user registration and sign-in](#register-or-sign-in-a-user) API with a stable `open_id`. The response includes `member_id`, `token`, and `refresh_token`. Broker stores the user mapping and passes the initial credentials to Broker App, which sets them in WhaleAppSDK or WhaleCore.
  </Accordion>
  ...
  <Accordion title="Are tokens client-specific?">
    No. Pass the initial token to Broker App and set it in WhaleAppSDK or WhaleCore. The SDK layer performs subsequent validity checks and automatic renewal; Broker App does not call check token or implement refresh logic. No party may write a complete token to logs.
  </Accordion>
</AccordionGroup>
