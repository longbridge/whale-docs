...
<CardGroup cols={2}>
  <Card title="Broker-managed users" icon="users">
    User data stays in the Broker system. BrokerAPI links the Client to a Whale user and brokerage account only when account opening begins.
  </Card>
  <Card title="Whale user system" icon="key">
    Broker performs the initial login handoff and sets the Whale credentials in WhaleSDK or WhaleCore. The SDK layer manages subsequent validation and renewal.
  </Card>
</CardGroup>
...
Broker Server performs the initial user login or linking and securely hands the returned credentials to Broker App. Broker App sets `token` and `refresh_token` in WhaleSDK; a Broker App implementing the securities UI with WhaleCore sets them in WhaleCore. The WhaleSDK layer manages the credentials thereafter.
...
<Steps>
  ...
  <Step title="Set credentials in the WhaleSDK layer">
    Broker App sets the initial credentials in WhaleSDK during initialization, or in WhaleCore when using WhaleCore. Broker App does not call check token itself.
  </Step>
  <Step title="Let the SDK validate and renew automatically">
    WhaleSDK or WhaleCore checks the token lifetime and renews it automatically with the refresh token. As long as renewal continues to succeed, Broker can treat the Client session as long-lived.
  </Step>
...
<Warning>
Broker App must not implement its own token-expiry checks, check-token call, or refresh-token renewal, and must not maintain a second refresh state alongside WhaleSDK or WhaleCore. Broker Server must still verify ownership and authorization for resources it handles directly.
</Warning>
