...
2. [Integration options](/en/docs/integration-options), with clear boundaries among WhaleAppSDK, WhaleCore + TradingAPI, and BrokerAPI.
...
| Users and sessions | New and existing Clients, SDK renewal, and logout | Stable `open_id` to `member_id` mapping and expected session invalidation |
...
- Login for a new and an existing Client.
...
| Preconditions | Environment, Client, account, market, and configuration |
...
- Client, user, application, and account mappings are queryable and traceable.
