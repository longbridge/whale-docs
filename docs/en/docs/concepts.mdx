...
| Client session | `token`, `refresh_token`, `sid` | Whale | Set the initial credentials in WhaleSDK or WhaleCore; the SDK layer validates and renews them |
...
- `token` represents the Client session. WhaleSDK or WhaleCore validates and renews it automatically; Broker App handles only unrecoverable invalidation events. A token does not replace Broker Server resource-ownership checks.
...
| --- | --- | --- | --- |
| Broker App to Whale | Broker App or web container | One Client | WhaleSDK UI, or WhaleCore + TradingAPI when implementing the securities UI from scratch |
