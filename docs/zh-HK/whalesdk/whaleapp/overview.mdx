...
WhaleAppSDK for iOS 和 WhaleAppSDK for Android 向 Broker App 提供完整的證券業務圖形介面。Broker App 不需要重新實現行情、資產和交易頁面，主要負責整合 SDK、傳入 Customer 會話、承接系統能力及管理 App 生命週期。
...
- WhaleAppSDK 自動續期無需 Broker App 介入；不可恢復的 token 失效、異地登入和 Customer 切換均有明確處理。
