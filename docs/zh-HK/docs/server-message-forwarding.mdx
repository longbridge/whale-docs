...
<Steps>
  ...
  <Step title="雙方完成訊息聯調">
    建議驗證正常訊息、重複 `post_id`、鑑權失敗、介面超時、返回失敗和多客戶訊息；實際範圍由雙方確認。
  </Step>
  <Step title="Broker App 驗收">
    驗證前臺、後臺、冷啟動、客戶登出、通知許可權關閉和安全跳轉。
  </Step>
</Steps>

## HTTP 介面約定 {/* http-endpoint-convention */}
...
| 欄位 | 型別 | 說明 |
| --- | --- | --- |
| `member_ids` | `string[]` | Whale 客戶標識列表，Broker 需映射到自己的使用者與裝置 |
...
| `pre_params` | `object` | Whale 內建預設變數，可能包含客戶資料 |
| `user_info` | `object` | 透傳給 Broker App 與 WhaleAppSDK 的訊息資料 |

### 通用請求示例 {/* generic-request */}
...
## 完整訂單訊息示例 {/* complete-order-message-example */}
...
## SDK 訊息對接格式 {/* whaleappsdk-message-format */}
...
## 已有模板檔案 {/* existing-template-files */}
