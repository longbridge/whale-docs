...
flowchart TD
    ...
    B --> C[接入方式選擇]
    C --> D{選擇技術路徑}
    D -->|現成 UI| E[WhaleAppSDK]
    D -->|自行實現證券功能 UI| F[WhaleCoreSDK + TradingAPI]
    D -->|機構級服務端能力| G[BrokerAPI]
    C --> H[使用者系統接入]
    H --> I{專案是否包含開戶}
    I -->|是| J[使用者綁定與開戶]
    I -->|否| K[跳過開戶工作流]
...
| 階段 | 指南 |
| --- | --- |
| 理解客戶、使用者、開戶申請、帳戶和訊息的關係 | [核心物件與業務流程](/zh-hk/docs/concepts) |
| 確定 UI、客戶端和服務端方案 | [接入方式選擇](/zh-hk/docs/integration-options) |
| 確定客戶註冊、登入和會話歸屬 | [使用者系統接入](/zh-hk/docs/user-system-integration) |
| 整合完整 UI | [WhaleAppSDK（iOS、Android、WebTrade）](/zh-hk/whalesdk/whaleapp/overview) |
| Broker App 自行實現證券功能 UI | [WhaleCoreSDK 與 TradingAPI](/zh-hk/trading-api/overview) |
| 接入機構級服務端能力 | [BrokerAPI](/zh-hk/broker-api/overview) |
| 專案包含開戶時，建立使用者、申請和證券帳戶映射 | [使用者綁定與開戶](/zh-hk/docs/account-opening) |
| 專案需要 App 訊息時，選擇並實現訊息鏈路 | [訊息推送接入](/zh-hk/docs/message-push) |
| 執行測試、驗收和生產切換 | [聯調、UAT 與上線](/zh-hk/docs/implementation) |
| 上線後的錯誤、事件和升級 | [運維與支援](/zh-hk/docs/operations-support) |

## 階段、交付物與完成條件 {/* phases-deliverables-and-exit-criteria */}
...
| Broker App Developer | WhaleAppSDK（iOS、Android、WebTrade）或 WhaleCoreSDK 整合 |
...
| 工作 | Broker | Whale |
| --- | --- | --- |
| Broker 產品範圍和客戶體驗 | 決策並驗收 | 提供能力邊界與實施建議 |
...
- **接入邊界**：採用 WhaleAppSDK、WhaleCoreSDK + TradingAPI 或 BrokerAPI；由哪一方維護客戶使用者體系。
...
接入產品的選擇見 [接入方式選擇](/zh-hk/docs/integration-options)。

調研結束時應形成一份“方案確認記錄”，至少寫明：

- Broker App 使用 WhaleAppSDK（iOS、Android、WebTrade），還是使用 WhaleCoreSDK + TradingAPI 自行實現證券功能 UI。
- Broker Server 需要使用哪些 BrokerAPI 能力。
- 客戶使用者由誰管理，`open_id` 如何生成和保持穩定。
- 開戶申請如何提交、查詢和展示失敗結果。
- 訊息由 Whale 託管渠道傳送，還是轉發到 Broker Server。
- 哪些能力本期不接入，後續如何處理。

## 3. 環境與資料準備 {/* 3-environment-and-material-preparation */}
...
### 環境準備完成條件 {/* environment-exit-criteria */}
...
## 4. 開發、聯調與驗收 {/* 4-development-integration-and-acceptance */}
...
3. 跑通資金、交易和狀態查詢等核心場景。
4. 按 [訊息推送接入](/zh-hk/docs/message-push) 驗證裝置通知或 Server-to-Server 訊息轉發。
5. 驗證非同步處理、重試、重複請求和失敗恢復。
6. 由業務團隊執行 UAT 並記錄驗收結果。

詳細的技術流程見 [實施與上線](/zh-hk/docs/implementation)。

### 聯調階段必須保留的證據 {/* evidence-to-retain */}
