...
flowchart TD
    ...
    D -->|自行實現證券功能 UI| F[WhaleCore + TradingAPI]
...
| Broker App 自行實現證券功能 UI | [WhaleCore 與 TradingAPI](/zh-hk/trading-api/overview) |
...
| Broker App Developer | WhaleAppSDK（iOS、Android、WebTrade）或 WhaleCore 整合 |
...
- **接入邊界**：採用 WhaleAppSDK、WhaleCore + TradingAPI 或 BrokerAPI；由哪一方維護客戶使用者體系。
...
- Broker App 使用 WhaleAppSDK（iOS、Android、WebTrade），還是使用 WhaleCore + TradingAPI 自行實現證券功能 UI。
