...
| 客戶會話 | `token`、`refresh_token`、`sid` | Whale | 將初始憑證設定到 WhaleAppSDK / WhaleCore；校驗與續期由 SDK 層管理 |
...
- `token` 表示客戶會話。WhaleAppSDK / WhaleCore 負責有效性檢查和自動續期；Broker App 僅處理不可恢復的失效事件。token 不會替代 Broker Server 的資源歸屬校驗。
...
| 鏈路 | 執行位置 | 身份邊界 | 主要用途 |
| --- | --- | --- | --- |
| Broker App 與 Whale | Broker App / Web 容器 | 單個客戶 | WhaleAppSDK UI，或 WhaleCore + TradingAPI 自行實現證券功能 UI |
