...
| --- | --- | --- | --- |
| 客戶 | `open_id` | Broker | 為同一客戶提供穩定且唯一的標識，不得複用 |
| Whale 使用者 | `member_id` | Whale | 儲存與 `open_id` 的映射，並校驗資源歸屬 |
| 客戶會話 | `token`、`refresh_token`、`sid` | Whale | 將初始憑證設定到 WhaleAppSDK / WhaleCoreSDK；校驗與續期由 SDK 層管理 |
...
<Warning>
標識存在不等於呼叫合法。Broker 服務端必須校驗 `member_id`、`application_id`、`account_no` 和 `order_id` 是否屬於當前客戶或當前 Broker 的授權範圍。
</Warning>
...
- `token` 表示客戶會話。WhaleAppSDK / WhaleCoreSDK 負責有效性檢查和自動續期；Broker App 僅處理不可恢復的失效事件。token 不會替代 Broker Server 的資源歸屬校驗。
...
4. 重複響應或重複訊息如何去重。
5. 最終失敗時向客戶展示什麼資訊、由誰處理。

## 三條技術鏈路 {/* three-technical-paths */}

| 鏈路 | 執行位置 | 身份邊界 | 主要用途 |
| --- | --- | --- | --- |
| Broker App 與 Whale | Broker App / Web 容器 | 單個客戶 | WhaleAppSDK UI，或 WhaleCoreSDK + TradingAPI 自行實現證券功能 UI |
...
<Note>
OpenAPI 面向 Broker 的開發者，用於策略交易、量化分析和 AI 應用，不是 Broker 實施團隊建設機構級整合的替代方案。
</Note>
