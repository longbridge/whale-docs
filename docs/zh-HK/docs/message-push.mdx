...
Whale 的訂單、帳戶及其他業務訊息可以透過兩種方式送達 Customer：由 Whale 管理雲推送渠道並直接投遞到裝置，或先透過 Server-to-Server 介面轉發給 Broker，再由 Broker 自行決定是否通知及如何分發。

## 完整接入文件

本頁用於說明總體方案和責任邊界。原始對接材料中的配置項、欄位和完整示例分別保留在以下頁面：

- [訊息推送方案選擇](/zh-Hant/docs/message-push-options)
- [推送渠道資訊收集](/zh-Hant/docs/push-channel-credentials)
- [Broker 自有推送渠道](/zh-Hant/docs/server-message-forwarding)
...
| 客戶端工作 | 註冊裝置，並將推送訊息交給 WhaleApp SDK | 註冊自有渠道，並將標準訊息體交給 WhaleApp SDK |
...
5. Broker App 完成系統通知許可權、裝置 token 註冊和 WhaleApp SDK 訊息處理。
...
sequenceDiagram
    ...
    Center->>Channel: 投遞到目標 Customer 的裝置
    Channel-->>App: title、body、user_info
    App->>App: 將標準訊息體交給 WhaleApp SDK
```
...
| 欄位 | 說明 | Broker 處理建議 |
| --- | --- | --- |
| `member_ids` | Whale Customer 標識列表 | 映射到 Broker 使用者和裝置，不要直接作為展示內容 |
...
| `user_info` | 客戶端透傳資料 | 推送到 App 後原樣交給 WhaleApp SDK |
...
無論選擇哪一種服務端路徑，最終交給 WhaleApp SDK 的訊息結構一致：
...
- 單個與多個 `member_ids` 均能正確映射目標 Customer。
- 同一 `post_id` 重複投遞只產生一次使用者可見通知。
- App 位於前臺、後臺、被系統終止時均有明確行為。
- Customer 登出、切換帳戶或關閉通知許可權後不會錯誤投遞。
- 簡體中文、繁體中文、英文及預設語言符合產品約定。
- 通知點選能進入正確頁面，無效或過期連結能夠安全降級。
- Broker 接收介面超時、返回失敗及下游廠商失敗均可觀測。
- 金鑰輪換後舊金鑰按計劃失效，且訊息投遞不中斷。

訊息推送應作為上線驗收的一部分，連同帳戶、資金和交易主流程一起完成生產就緒檢查。完整交付階段見[客戶接入與交付流程](/zh-Hant/docs/broker-onboarding)。
