...
| 客户端工作 | 注册设备，并将推送消息交给 WhaleSDK | 注册自有渠道，并将标准消息体交给 WhaleSDK |
...
5. Broker App 完成系统通知权限、设备 token 注册和 WhaleSDK 消息处理。
...
| `user_info` | 客户端透传数据 | 推送到 App 后原样交给 WhaleSDK |
...
无论选择哪一种服务端路径，最终交给 WhaleSDK 的消息结构一致：
