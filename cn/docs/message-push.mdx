...
- [消息推送方案选择](/cn/docs/message-push-options)
- [推送渠道信息收集](/cn/docs/push-channel-credentials)
- [Broker 自有推送渠道](/cn/docs/server-message-forwarding)
...
| 客户端工作 | 注册设备，并将推送消息交给 WhaleApp SDK | 注册自有渠道，并将标准消息体交给 WhaleApp SDK |
...
5. Broker App 完成系统通知权限、设备 token 注册和 WhaleApp SDK 消息处理。
...
| `user_info` | 客户端透传数据 | 推送到 App 后原样交给 WhaleApp SDK |
...
无论选择哪一种服务端路径，最终交给 WhaleApp SDK 的消息结构一致：
...
消息推送应作为上线验收的一部分，连同账户、资金和交易主流程一起完成生产就绪检查。完整交付阶段见[客户接入与交付流程](/cn/docs/broker-onboarding)。
