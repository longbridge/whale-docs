...
<Accordion title="原始对接问题（英文）">
  - Authentication details: credentials, request authentication, token and header handling, session duration, refresh and client identification.
  - WebSocket connection, protocol, session details and event types.
  - Use cases showing which endpoints are combined for a business scenario.
  - Explanations of each endpoint or API collection and when to use it.
</Accordion>
...
以下代码保留原始对接材料中的完整示例。运行前需要安装 Go，并把地址和凭证替换为项目实际配置。
...
<AccordionGroup>
  <Accordion title="Broker API 的访问限频是多少？">
    历史材料记录为按 API Key 总计每秒 `200` 次。限频可能按环境和项目调整，上线前应以当前 Broker API 契约和 Whale 项目团队确认结果为准。
  </Accordion>
