...
WhaleSDK 支持两种消息对接方案。两种方案最终都由 Broker App 接收标准消息体并交给 WhaleSDK，区别在于消息由 Whale 直接投递到云推送渠道，还是先转发给 Broker Server。
...
<Steps>
  ...
  <Step title="集成 Broker App">
    Broker App 对接 WhaleSDK 消息 API，并验证通知权限、设备 token、前后台接收和点击跳转。
  </Step>
</Steps>
...
<CardGroup cols={2}>
  ...
  <Card title="需要考虑" icon="code">
    Broker Server 需要实现接收、鉴权、幂等、可靠落库、监控和下游分发。
  </Card>
</CardGroup>
...
<Steps>
  ...
  <Step title="集成 Broker App">
    Broker App 将最终收到的标准消息体交给 WhaleSDK，并完成安全跳转和兼容处理。
  </Step>
</Steps>
