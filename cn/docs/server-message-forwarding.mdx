...
| `user_info` | `object` | 透传给 Broker App 与 WhaleApp SDK 的消息数据 |
...
Broker App 从自有推送渠道取得消息后，需要保留 `title`、`body` 和完整 `user_info`，并交给 WhaleApp SDK 的消息处理入口。

```jsonc
{
    "title": "消息标题", // WhaleApp SDK 需要
    "body": "消息摘要",
    "user_info": {
        "t": "xx", // 消息类型或埋点字段
        "tn": "xx", // 追踪或埋点字段
