...
Whale 通过 Server-to-Server 接口把标准业务消息转发给 Broker Server。Broker 负责鉴权、幂等、可靠落库、用户映射和下游分发。
...
<Steps>
  ...
  <Step title="双方完成消息联调">
    验证正常消息、重复 `post_id`、鉴权失败、接口超时、返回失败和多 Customer 消息。
  </Step>
...
| Header | 必填 | 说明 |
| --- | --- | --- |
| `X-Api-Key` | 是 | Broker 提供给 Whale 的接口认证 Key |
| `X-Trace-Id` | 是 | 端到端链路追踪标识，双方日志均需记录 |
...
`code` 为 `0` 表示成功，非 `0` 表示失败。原始方案注明失败消息目前不会重试，因此 Broker 接口应在完成鉴权、校验和可靠落库后尽快返回成功；实际重试策略应在项目上线前再次确认。
...
```jsonc
{
    ...
    "user_info": {
        ...
        "tn": "xx", // 追踪或埋点字段
        "id": "xx", // 消息标识
        "lb_push_from": "whale", // 标识 Whale 消息
        "link": "跳转链接",
        "json": "业务内容 JSON 字串"
    }
}
