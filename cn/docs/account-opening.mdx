...
<AccordionGroup>
  <Accordion title="如何打通两边的用户？token 和 refresh_token 如何获得？">
    使用稳定的 `open_id` 调用[用户注册登录](#用户注册登录)接口。接口返回 `member_id`、`token` 和 `refresh_token`；Broker 保存用户映射，Broker App 保存会话凭证。
  </Accordion>
  ...
  <Accordion title="Token 是否区分客户端？">
    不区分。取得 token 后应提供给 Broker App 使用。Broker 服务端是否保存凭证应遵循项目的会话与安全方案，不能在日志中记录完整 token。
  </Accordion>
</AccordionGroup>
