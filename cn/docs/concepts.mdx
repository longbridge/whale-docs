...
| Customer 会话 | `token`、`refresh_token`、`sid` | Whale | 将初始凭证设置到 WhaleSDK / WhaleCore；校验与续期由 SDK 层管理 |
...
- `token` 表示 Customer 会话。WhaleSDK / WhaleCore 负责有效性检查和自动续期；Broker App 仅处理不可恢复的失效事件。token 不会替代 Broker Server 的资源归属校验。
...
| 链路 | 运行位置 | 身份边界 | 主要用途 |
| --- | --- | --- | --- |
| Broker App 与 Whale | Broker App / Web 容器 | 单个 Customer | WhaleSDK UI，或 WhaleCore + Trading API 自行实现证券功能 UI |
