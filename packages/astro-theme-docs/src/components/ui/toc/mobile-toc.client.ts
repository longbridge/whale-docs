 ...
 *
 * Ported from Kumo's `layout="select"` TableOfContents scroll-spy:
 *
 ...
 *
 * Adapted for nimbus: teardown via AbortController for view transitions, a
 * persistent in-band set (like the desktop rail in `toc.client.ts`) so the
 * active heading is stable when several fall inside the band at once, and
...
// reading band is therefore [10%, 30%] of the viewport height; BAND_TOP is
// its upper edge, reused by the first/last clamp below.
const BAND_TOP = 0.1;
