...
export function Scene<N extends string, E extends string>({
	...
	const node = (id: N, label: ReactNode, opts?: SceneNodeOptions) => {
		...
		return (
			<div
				...
				ref={setNode(id)}
				className={cn(
					"relative z-10 text-center font-mono font-medium uppercase select-none",
					chip
						? "px-2 py-1 text-[10px] tracking-wider text-neutral-700 dark:text-neutral-300"
						: cn(
								"px-2.5 py-1.5 text-xs transition-colors duration-400 sm:px-4 sm:py-2 sm:text-sm",
								activeNode === id
	...
	return (
		<div
			ref={containerRef}
			className={cn("relative w-full p-3 md:p-10", className)}
		>
			<DiagramDefs />
			<svg
			...
			>
				{(Object.entries(rects) as [N, EdgeRect][]).map(([id, rect]) => {
					...
					if (opts?.shape)
						return <g key={`node-${id}`}>{opts.shape(rect, isActive)}</g>;
					...
					return (
						<rect
							...
							style={{
								transition:
									"stroke 400ms ease-out, stroke-width 400ms ease-out",
							}}
						/>
					);
				})}
