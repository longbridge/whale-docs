...
import {
	...
	type NodeRect,
	type NotchConfig,
} from "@/components/react/diagram/welding";
...
const PARTS = [
	{
		id: "channels",
		label: "Channels",
		href: "/agents/communication-channels/",
	},
...
const NODE_NOTCHES: Record<PartId, NotchConfig> = (() => {
	...
	for (const [id, sides] of Object.entries(map) as [
		PartId,
		Record<Side, number[]>,
	][]) {
...
function AgentPrimitivesBody() {
	...
	const selector = useCallback((c: HTMLDivElement) => {
		...
		return {
			rects,
			wrap: wrapperRef.current ? box(wrapperRef.current) : EMPTY_RECT,
		};
	}, []);
	...
	if (hasRects) {
		...
		for (const s of STUBS) {
			...
			if (a)
				segments.push({
					d: orthPath(a, toBoundary(a, s[1]), s[1]),
					dots: [a],
					opacity: 0.2,
				});
		}
							...
							{hasRects && (
								<>
									...
									<text
										...
										className="fill-neutral-800 font-mono text-[10px] font-medium uppercase select-none dark:fill-neutral-200"
									>
										{labelText}
									</text>
						...
						>
							{PARTS.map((p) => (
								<a
									...
									style={{ gridArea: p.id }}
									className="flex h-[40px] w-[112px] items-center justify-center no-underline sm:h-[44px] sm:w-[148px]"
								>
									<span
										className={cn(
											"relative z-10 px-2 py-1 text-center font-mono text-[11px] font-medium transition-colors duration-500 select-none sm:px-3 sm:py-1.5 sm:text-[13px]",
											composed
